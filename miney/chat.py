...
class Chat:
    ...
    def send_to_all(self, message: str) -> None:
        ...
        :return: None
        """
        self.mt.lua.run(f"minetest.chat_send_all('{message}')")

    def send_to_player(self, playername: str, message: str):
        return self.mt.lua.run(
            f"return minetest.chat_send_player('{playername}', '{message}')"
        )

    def format_message(self, playername: str, message: str):
        return self.mt.lua.run(
            f"return minetest.format_chat_message('{playername}', '{message}')"
        )
